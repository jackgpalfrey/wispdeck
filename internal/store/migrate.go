...
func migrationFourteen(ctx context.Context, tx *sql.Tx) error {
	// Retired links remain immutable historical records, but their public name
	// may now point at a fresh link owned by the same user. The old global UNIQUE
	// constraint on short_links.slug prevented that history-preserving model.
	const schema = `
		DROP INDEX short_links_owner_created;
		DROP INDEX short_link_audit_owner_time;

		ALTER TABLE short_link_destinations RENAME TO short_link_destinations_v13;
		ALTER TABLE short_link_daily_stats RENAME TO short_link_daily_stats_v13;
		ALTER TABLE short_link_audit_events RENAME TO short_link_audit_events_v13;
		ALTER TABLE short_links RENAME TO short_links_v13;
		...
		SELECT id, owner_user_id, slug, title, description, mode, enabled,
		       created_at, updated_at, expires_at, deleted_at
		FROM short_links_v13;

		CREATE UNIQUE INDEX short_links_active_slug
			ON short_links(slug)
		...
		INSERT INTO short_link_destinations (id, link_id, position, label, target_url)
		SELECT id, link_id, position, label, target_url
		FROM short_link_destinations_v13;
		...
		SELECT link_id, day, visits, last_visited_at
		FROM short_link_daily_stats_v13;
		...
		INSERT INTO short_link_audit_events (
			id, occurred_at, actor_user_id, owner_user_id, link_id, slug, kind
		)
		SELECT id, occurred_at, actor_user_id, owner_user_id, link_id, slug, kind
		FROM short_link_audit_events_v13;
		CREATE INDEX short_link_audit_owner_time
			ON short_link_audit_events(owner_user_id, occurred_at DESC, id DESC);

		DROP TABLE short_link_destinations_v13;
		DROP TABLE short_link_daily_stats_v13;
		DROP TABLE short_link_audit_events_v13;
		DROP TABLE short_links_v13;
	`
